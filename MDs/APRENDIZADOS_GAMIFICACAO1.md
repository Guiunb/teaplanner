...
## Correção crítica: crash "can't redefine allCards" (03/07/2026, v2)
- **Causa:** Gemini mudou `let allCards`/`let boardsMeta` para `var` no core.js. `var` global cria propriedade não-configurável no window; o bloco de exposição ao Test Runner (`Object.defineProperty`) então lança TypeError ANTES do `initApp()`, matando toda a inicialização (só o HTML estático aparecia).
- **Correção:** os dois `defineProperty` do init.js agora estão em try/catch — funciona com `var`, `let` ou `const`, hoje e no futuro.
- **Lição:** blocos de "exposição para testes" devem ser sempre defensivos; e o print do console do usuário é a fonte de diagnóstico mais valiosa — pedir SEMPRE.
