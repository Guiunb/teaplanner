...
const htmlPath = 'c:/Users/Guilherme/Dropbox/- TEA PLANNER 2.0/index.html';
let content = fs.readFileSync(htmlPath, 'utf8');

// 1. Update Dropdown and Header button CSS styles
const oldDropdownCss = `        /* Styling for Header Dropdowns */
        .header-dropdown {
            position: relative;
...
const newDropdownCss = `        /* Styling for Header Dropdowns - Premium Glassmorphism */
        :root {
            --header-text: #ffffff;
...
if (content.includes(oldDropdownCss)) {
    content = content.replace(oldDropdownCss, newDropdownCss);
} else {
    console.warn("Warning: oldDropdownCss block not found exactly. Trying fallback...");
}

// 2. Modify header button CSS to use adaptive header variables
const oldHeaderStyles = `        header.app {
            position: sticky;
            top: 0;
            z-index: 10;
            display: flex;
...
const newHeaderStyles = `        header.app {
            position: sticky;
            top: 0;
            z-index: 10;
            display: flex;
...
if (content.includes(oldHeaderStyles)) {
    content = content.replace(oldHeaderStyles, newHeaderStyles);
} else {
    console.warn("Warning: oldHeaderStyles block not found exactly.");
}

// 3. Fallback and direct replacement for garbled menu characters
// Replacing Board Controls dropdown structure
const oldBoardDropdown = `                        <div class="header-dropdown" id="boardDropdownContainer">
...
const newBoardDropdown = `                        <div class="header-dropdown" id="boardDropdownContainer">
...
if (content.includes(oldBoardDropdown)) {
    content = content.replace(oldBoardDropdown, newBoardDropdown);
} else {
    // Fallback individual replaces
    content = content.replace(/Quadros â–¾/g, "Quadros &#9662;");
    content = content.replace(/Dados â–¾/g, "Dados &#9662;");
    content = content.replace(/âž• Adicionar Quadro/g, "➕ Adicionar Quadro");
    content = content.replace(/âœ ï¸  Renomear Quadro/g, "✏️ Renomear Quadro");
    content = content.replace(/ðŸ’¾ Salvar Quadro como\.\.\./g, "💾 Salvar Quadro como...");
    content = content.replace(/ðŸŽ¨ Cor do Quadro/g, "🎨 Cor do Quadro");
    content = content.replace(/ðŸ—‘ï¸  Excluir Quadro/g, "🗑️ Excluir Quadro");
    content = content.replace(/ðŸ—‘ Excluir Quadro/g, "🗑️ Excluir Quadro");
    content = content.replace(/ðŸ“¥ Salvar Json/g, "📥 Salvar Json");
    content = content.replace(/ðŸ“¤ Importar Json/g, "📤 Importar Json");
}

// 4. Replacing List Context Menu
const oldListCtx = `        <div id="ctx-list" class="ctx">
...
const newListCtx = `        <div id="ctx-list" class="ctx">
...
if (content.includes(oldListCtx)) {
    content = content.replace(oldListCtx, newListCtx);
} else {
    // Fallback individual replaces
    content = content.replace(/ðŸ—‘ï¸  Excluir lista/g, "🗑️ Excluir lista");
    content = content.replace(/ðŸ—‘ï¸  Excluir TODOS desta Lista/g, "🗑️ Excluir TODOS desta Lista");
    content = content.replace(/âž¡ï¸  Mover TODOS desta Lista/g, "➡️ Mover TODOS desta Lista");
    content = content.replace(/âž¡ï¸  Mover LISTA para outro quadro/g, "➡️ Mover LISTA para outro quadro");
}

// 5. Replace theme logic to apply contrast rules
const oldThemeFunction = `            function setBoardTheme(color) {
...
const newThemeFunction = `            function getContrastYIQ(hexcolor){
...
if (content.includes(oldThemeFunction)) {
    content = content.replace(oldThemeFunction, newThemeFunction);
} else {
    console.warn("Warning: oldThemeFunction block not found exactly.");
}

// Write the file back as UTF-8
fs.writeFileSync(htmlPath, content, 'utf8');
console.log("HTML successfully updated with premium styles and fixed unicode emojis!");
